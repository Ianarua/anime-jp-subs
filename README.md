...
> **断句是听日语自己断的**，不需要中文字幕轨：句子在哪儿断由识别模型自己判断（它的断句比翻译
> 字幕的断句贴近原声），每句的起止时间再用人声检测（silero VAD）量出来的换气停顿对齐，
> 最后按「一行放得下」拼成一条条字幕。所以哪怕片源只有画面和声音也能跑，见下面「挑片源的小技巧」。
>
...
run_jp_sub.bat review "D:\Anime\2026.7"   :: 生成"评审页"：逐条检查断句对不对
...
字幕的**断句**是对是错，机器测不出来，只能靠耳朵。所以留了一个评审工具：

```bat
run_jp_sub.bat review "D:\Anime\2026.7\某番"    :: 已有评审页就跳过，想重做加 --force
```

它会在每集旁边生成两个文件（**不改动 mkv，删掉即还原**）：
...
| ⚠行首助詞 | 这一行从助词开头（断点很可能该往前挪） |
| ⚠行尾起句 | 行尾是「いや / あの / え」这种只会起句的词（应该归下一行） |
| ⚠偏长 | 一条显示超过 8 秒或 24 字 |

**为什么值得花十分钟标一遍**：这份人工标记可以直接当"验收标准"用——这个项目的断句算法
就是靠一集 80 条标记，从"只对 0 处"改到"对 44 处"的；你标出来的漏句，后来也变成了
自动补漏那一步的依据。

### 打分：拿标记给切分算法打分（**只有 dev 分支有**）

`score` 是开发/调参用的，用户版（`main`）里没有。它需要三样东西：标记文本、**人当时审的
那份产物**（`--baseline`，标记的行号属于它）、那一集的听写结果（`--dump`）：
...
口径（钉死了，别改）：**合并**标记 = 那个词缝上不该有切点，**拆分**标记 = 该有切点，
允许差一个词缝。`-v` 会把没通过的逐条列出来（带时间码，方便回听）。
...
4. **断句 + 注音**：先把识别出来的每句话（模型自己断的句）对齐到 VAD 量出来的换气停顿上，
   再按「一行放得下」拼成一条条字幕（日语自己断句，不看中文字幕轨）；然后查词典标平假名，
   排版成 ASS（含字体信息）。
5. **内封 + 自检 + 替换**：
...
字幕文本来自语音识别，识别错的地方注音也会跟着错。断句是听日语自己的换气停顿来的，
碰到「连着说没有明显停顿」的地方偶尔会和原声不完全一致。这种情况可以反馈具体的集数和时间点。
...
- **中文轨和音频不同步的地方，日语也不会凭空补上**：官方中文轨偶尔会错位，
  脚本按实测的语音时间归属，所以可能出现「有中文、没日语」的片段。
- **断句靠语音识别 + 换气停顿**：说得又快又没有停顿的地方（战斗喊叫、一口气念完的台词），
  断句可能和你的语感不一致；先按「一行放得下」保证不折行、不糊在屏幕上。
- **没有中文字幕轨也能跑**：日语断句不看中文轨。中文轨只决定「保留哪条中文轨」，
  连一条字幕轨都没有时，成品里就只有日语轨。
...
- **优先挑自带中文内封字幕的**：这样成品里日语 + 中文两条都在，随时能对照。标题里带「简繁」
  「SC/TC」「简繁日」的通常都有。只有日文/英文字幕的片源也能跑（日语断句不看中文轨），
  只是成品里没有中文轨可切。
...
README 讲"怎么用"，下面几篇讲"为什么这么设计"，结论都带实测数据：
