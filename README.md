...
> ⚠️ **硬性前提：片源必须自带中文字幕轨（能被提取出来的那种内封字幕）。**
> 日语字幕的断句是**照着中文轨切的**（中文意译的断句最接近播报节奏）；没有中文轨时断句质量会明显变差，
> 只能退化成拿第一条字幕轨当基准（连一条字幕轨都没有时，就只剩语音识别自己的断句）。
> 所以挑片源时先确认「有中文内封字幕」，见下面「挑片源的小技巧」。
...
4. **断句对齐 + 注音**：按**中文字幕轨的断句边界**把结果切成一句一句（所以片源必须有中文轨，
   没有的话会打一行 `[warn] 没有中文语言标签的字幕轨，退回第一条` 然后拿第一条当基准）；
   先生成日语文本，再查词典标平假名，排版成 ASS（含字体信息）。
5. **内封 + 自检 + 替换**：
...
- **必须要有中文字幕轨**：日语字幕的断句是照着中文轨切的。没有中文轨时不会报错，但基准会退化成
  「第一条字幕轨」（连一条字幕轨都没有时，只能按语音识别自己的断句走），断句和长句质量会明显变差。
...
- **先确认有中文字幕轨**：这是硬性前提（断句基准）。标题里带「简繁」「SC/TC」「简繁日」的通常
